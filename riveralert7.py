...
riverLevels = [[html[i-1],html[i], html[i+1]] for i in range(len(html)) if (i-2)%5 == 0 and func.is_number(html[i])] #omg a list comprehension
...
for i in range(len(riverLevels)):
    ...
    if not river.strip() in badrivers:
        ...
        if excess >= 0:
##            #weekend excess
##            timeToWeekend=259200 #number of seconds in 3 days (60*60*24*3)
##            weekendLevel=func.newLevel(river
##            riverDropString=river+' could drop to '+weekendLevel+'by the weekend\n'
            
            message = message + '\n\n' + river + liklihood + 'It is ' + str(excess) + 'm above the minimum level.\n[' +str(level) + 'm/' + str(req) + 'm]\n' + links
            
      
TO = ['herbertjemma@gmail.com', 'da_winkel@hotmail.com']
